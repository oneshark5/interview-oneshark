...
1. path 记录路径、res 存放结果
2. 每访问到一个节点就加入路径并求和，当到达叶子节点，并且 sum === target，就加入结果数组
...
- 深度优先遍历：
  ...
  - 为了同一个单元格内的字母不允许被重复使用，遍历到某字符后，将当前字符设置为`''`，防止四个方向 dfs 再次遍历到
  - 四个方向遍历完毕后，再恢复这个字符
- 最后一只匹配失败，返回`false`
...
[参考飞鸟 🐦](https://github.com/lzxjack/coding-interviews/blob/master/problems/13.%20%E6%9C%BA%E5%99%A8%E4%BA%BA%E7%9A%84%E8%BF%90%E5%8A%A8%E8%8C%83%E5%9B%B4.md)
...
const isTrue = (m, n, k) => {
  ...
  while (m !== 0) {
    // 因为是数位之和，所以大于10的数要分解
    sum1 += m % 10; // 取个位
    m = Math.floor(m / 10); // 取十位， 如果仅是个位数，会返回0
  }
  while (n !== 0) {
    sum2 += n % 10;
    n = Math.floor(n / 10);
...
**[剑指 Offer 03. 数组中重复的数字](https://github.com/lzxjack/coding-interviews/blob/master/problems/03.%20%E6%95%B0%E7%BB%84%E4%B8%AD%E9%87%8D%E5%A4%8D%E7%9A%84%E6%95%B0%E5%AD%97.md)**
...
**[剑指 Offer 15. 二进制中 1 的个数](https://github.com/lzxjack/coding-interviews/blob/master/problems/15.%20%E4%BA%8C%E8%BF%9B%E5%88%B6%E4%B8%AD1%E7%9A%84%E4%B8%AA%E6%95%B0.md)**

```js
var hammingWeight = function (n) {
  // 给定一个十进制整数，转成二进制然后统计二进制表示中 1 的个数
  console.log(n);
...
map 的遍历方法
采用`for...of...`遍历 map 集合的 key 值或 value 值；
map.keys()返回集合的所有 key
map.values()返回集合的所有 value

```js
var majorityElement = function (nums) {
  // 统计每个数字出现的次数，若次数超过数组长度的一半则返回结果
  const map = new Map();
  for (let num of nums) {
...
**[剑指 Offer 56 - I. 数组中数字出现的次数](https://github.com/lzxjack/coding-interviews/blob/master/problems/56%20-%20I.%20%E6%95%B0%E7%BB%84%E4%B8%AD%E6%95%B0%E5%AD%97%E5%87%BA%E7%8E%B0%E7%9A%84%E6%AC%A1%E6%95%B0.md)**
...
**[剑指 Offer 61. 扑克牌中的顺子](https://github.com/lzxjack/coding-interviews/blob/master/problems/61.%20%E6%89%91%E5%85%8B%E7%89%8C%E4%B8%AD%E7%9A%84%E9%A1%BA%E5%AD%90.md)**

思路：
- 遇 0 则跳过
- 采用 Set 集合去重
- 记录最大值、最小值， max - min > 5 return false

🐦思路
使用Set来判断数字是否重复，遍历nums，用两个变量维护牌的最大值和最小值，遍历时检查是否重复。最后判断最大值和最小值之差即可，具体如下：
- 大小王能代替任何数字，遇到大小王，直接跳过本次循环
- 更新最大值和最小值
- 若遇到重复的牌，肯定不是顺子了，返回false
- 最后判断最大值和最小值之差，若大于等于5，肯定不是顺子。因为已经去重，所以只要小于5，肯定是顺子。

```js
var isStraight = function (nums) {
  // ⭐⭐⭐注意：大小王可以代替任何数字
  // 遍历数组，所遍历到的数大于或等于前面的数则返回true 否则返回false ×××
  const set = new Set();
  let [max, min] = [0, 14];
  for (let num of nums) {
    if (num === 0) continue;
...
const constructArr = a => {
    // 思路：先记录左累积、右累积，即记录从左计算到某位置所得积和记录从右计算到某位置所得积
    const len = a.length
...
- 递归
  ...
  var lowestCommonAncestor = function(root, p, q) {
      // 递归
      // 思路：判断'根'节点与p q的关系，如果大于则向左子树查找，小于则向右子树查找，最后返回根节点
      if(!root) return root
...
const quickSort = (arr, k, l, r) => {
    ...
    // 以 left 为基准
    while(i < j){
        while(i < j && arr[j] >= arr[l]) j--; // 右边的数有比左边小的查找停止 记录
        while(i < j && arr[i] <= arr[l]) i++; // 同理  （就是小的放右边，）
        [arr[i], arr[j]] = [arr[j], arr[i]];
    }
    // 循环之后，交换l 和 i的位置，此时 i左边是小于它的数， 右边是大于它的数
...
var lengthOfLongestSubstring = function(s) {
    // 思路：滑动窗口？
    // 遇到重复字符将window清空，重新计数
    const win = [];
    let max = 0;
    const len = s.length;
    for(let i=0; i<len; i++){
        const index = win.indexOf(s[i]) // 遍历到的字符是否在窗口中
        if(index !== -1){
            // 表明窗口中已经存在该字符，所以我们需要清空窗口重新计算
            win.splice(0, index+1)
        }
        win.push(s[i])
        if(max < win.length) max = win.length;
    }
    return max;
};
```
...
**[剑指 Offer 51. 数组中的逆序对](https://github.com/lzxjack/coding-interviews/blob/master/problems/51.%20%E6%95%B0%E7%BB%84%E4%B8%AD%E7%9A%84%E9%80%86%E5%BA%8F%E5%AF%B9.md)**
...
**[剑指 Offer 59 - II. 队列的最大值](https://github.com/lzxjack/coding-interviews/blob/master/problems/59%20-%20II.%20%E9%98%9F%E5%88%97%E7%9A%84%E6%9C%80%E5%A4%A7%E5%80%BC.md)**
...
**[剑指 Offer 58 - I. 翻转单词顺序](https://github.com/lzxjack/coding-interviews/blob/master/problems/58%20-%20I.%20%E7%BF%BB%E8%BD%AC%E5%8D%95%E8%AF%8D%E9%A1%BA%E5%BA%8F.md)**
...
**[剑指 Offer 58 - II. 左旋转字符串](https://github.com/lzxjack/coding-interviews/blob/master/problems/58%20-%20II.%20%E5%B7%A6%E6%97%8B%E8%BD%AC%E5%AD%97%E7%AC%A6%E4%B8%B2.md)**
...
**[剑指 Offer 67. 把字符串转换成整数](https://github.com/lzxjack/coding-interviews/blob/master/problems/67.%20%E6%8A%8A%E5%AD%97%E7%AC%A6%E4%B8%B2%E8%BD%AC%E6%8D%A2%E6%88%90%E6%95%B4%E6%95%B0.md)**
